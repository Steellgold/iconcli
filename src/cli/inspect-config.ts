...
export const runInspectConfig = async (projectRoot: string): Promise<void> => {
  ...
  try {
    ...
    if (formatConfig.source && formatConfig.source !== "default") {
      ...
      logger.newline();
      logger.print(formatConfigForDisplay(formatConfig));
      logger.newline();
      logger.info("These rules will be applied to generated icon components.");
      logger.newline();
      logger.info("To disable auto-adaptation, use:");
      logger.print("  • CLI flag: mkicon -n Icon icon.svg --no-adapt");
      logger.print('  • Config file: Set "adaptToProject": false in .mkicon.json');
    } else {
      ...
      logger.print("  • Prettier  — .prettierrc.json  or  pnpm add -D prettier");
      logger.print("  • Biome     — biome.json         or  pnpm add -D @biomejs/biome");
      logger.print("  • ESLint    — .eslintrc.json     (with formatting rules)");
      logger.print("  • EditorConfig — .editorconfig");
    }
  } catch (error) {
